...
export default async function handler(
  ...
  try {
    console.log('🔄 Updating "Matchs du jour" with REAL live data...');
    ...
    const realLiveMatches = await footballAPI.makeRequest(`/matches?dateFrom=${todayStr}&dateTo=${tomorrowStr}&status=IN_PLAY,PAUSED,FINISHED`);
    ...
    console.log(`✅ Real live sync completed. Updated ${updatedGames.length} games for "Matchs du jour"`);

    return res.status(200).json({
      success: true,
      message: `Successfully updated ${updatedGames.length} "Matchs du jour" with REAL live data`,
      games: updatedGames,
