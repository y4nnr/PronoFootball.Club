...
import MockFootballDataAPI from '../../lib/mock-football-data-api';
...
export default async function handler(
  ...
  try {
    console.log('🧪 Testing live sync with MOCK external API...');

    // Initialize Mock Football-Data.org API
    const mockAPI = new MockFootballDataAPI();

    // Test API connection first
    const isConnected = await mockAPI.testConnection();
    if (!isConnected) {
      return res.status(500).json({
        error: 'Failed to connect to Mock Football-Data.org API',
        details: 'Please check your mock API and try again'
      });
    }
    ...
    // Get all relevant matches from Mock Football-Data.org in one call
    const allMatches = await mockAPI.getAllRelevantMatches();
    console.log(`📊 Found ${allMatches.length} total matches from MOCK external API`);
    ...
    for (const game of gamesToSync) {
      try {
        ...
        if (!hasGameDataChanged(currentData, newData)) {
          console.log(`⏭️ No changes for ${game.homeTeam.name} vs ${game.awayTeam.name}`);
    ...
    return res.status(200).json({
      success: true,
      message: `Successfully synced ${updatedGames.length} games with MOCK external API`,
      updatedGames,
