...
export default function BettingPage({ game, allGames, currentGameIndex }: BettingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 -mt-8 sm:mt-0">
        ...
        <div className="mb-0 sm:mb-3 md:mb-4">
          ...
          <div className="hidden md:block">
            <div className="flex flex-col items-center mb-1 md:mb-2">
              <div className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-0.5">Compétition</div>
              <div className="text-lg md:text-xl font-bold text-gray-900 text-center">{game.competition.name}</div>
            </div>
          </div>
        </div>
          ...
          <div className="p-2 sm:p-4 md:p-6">
            ...
            {allGamesList && allGamesList.length > 1 && (
              <div className="mb-3 sm:mb-4 md:mb-5">
                ...
                <div className="relative bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-5 border border-gray-200 shadow-md w-full group min-h-[160px] sm:min-h-[180px] md:min-h-[200px]">
                  {/* Mobile: Show 1-2 games, Desktop: Show 4-5 games */}
                  {/* Left Arrow */}
                  <button
                    onClick={() => scrollCarousel('left')}
                    disabled={!canScrollLeft}
                    className={`absolute left-1 sm:left-2 top-1/2 -translate-y-1/2 z-10 w-8 h-8 sm:w-10 sm:h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 ${
                      canScrollLeft 
                    ...
                    {allGamesList.map((gameItem, index) => {
                      ...
                      return (
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-[10px] sm:text-xs mb-1 font-medium text-gray-500">VS</div>
                          <div className="font-semibold text-[10px] sm:text-xs md:text-sm mb-2 truncate text-gray-900">
                            {gameItem.awayTeam.name}
                          </div>
                          
                          <div className="space-y-0.5 sm:space-y-1">
                            <div className="flex items-center justify-center space-x-0.5 sm:space-x-1 text-gray-600">
                          ...
                          {gameItem.bets && gameItem.bets.length > 0 && (
                            <div className="mt-1.5 sm:mt-2 p-0.5 sm:p-1 md:p-1.5 rounded bg-blue-100">
                              <div className="text-[10px] sm:text-xs font-semibold text-blue-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 md:space-y-5 transition-all duration-500 ease-in-out">
              {error && (
                <div className="p-2 sm:p-3 bg-red-100 text-red-700 rounded-md text-sm sm:text-base">
                  {error}
                </div>
              )}

              {success && (
                <div className="p-2 sm:p-3 bg-primary-100 text-primary-700 rounded-md flex items-center text-sm sm:text-base">
              ...
              <div className={`bg-gradient-to-br from-primary-50 to-primary-100 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 border shadow-md relative ${
                existingBet ? 'border-blue-500 border-2' : 'border-primary-200'
              }`}>
                {/* Checkmark indicator when bet is placed */}
                {existingBet && (
                  <div className="absolute -top-1 sm:-top-2 -right-1 sm:-right-2 w-6 h-6 sm:w-8 sm:h-8 bg-blue-500 rounded-full flex items-center justify-center shadow-lg z-10">
                ...
                <div className="text-center mb-1.5 sm:mb-2 md:mb-3">
                  <h3 className="text-base sm:text-lg md:text-xl font-bold text-gray-900 mb-1">Entrez votre pronostic</h3>
                  
                  {/* Date/Time Display - Top Center */}
                  <div className="flex items-center justify-center space-x-2 sm:space-x-4 mb-1.5 sm:mb-2 md:mb-3">
                    <div className="flex items-center space-x-1 sm:space-x-2 bg-white rounded-md sm:rounded-lg px-2 sm:px-3 md:px-4 py-1 sm:py-2 shadow-md border border-gray-200">
                ...
                {/* Game Display */}
                <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg p-2 sm:p-3 md:p-4 mb-2 sm:mb-3 border border-gray-200 shadow-sm">
                  <div className="flex items-stretch justify-between">
                    <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-1 min-h-[50px] sm:min-h-[60px] md:min-h-[70px]">
                      {game.homeTeam.logo && (
                        <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-full bg-white p-1 sm:p-1.5 md:p-2 shadow-md border border-gray-100 flex-shrink-0 self-center">
                          <img
                      ...
                      <div className="min-w-0 flex-1 flex flex-col justify-center">
                        <div className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-bold text-gray-900 break-words leading-tight min-h-[2em] flex items-center">{game.homeTeam.name}</div>
                        <div className="text-[10px] sm:text-xs text-gray-500 font-medium uppercase tracking-wide mt-1">Domicile</div>
                      </div>
                    </div>
                    
                    <div className="mx-2 sm:mx-4 md:mx-6 text-center flex-shrink-0 self-center">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-blue-100 rounded-full flex items-center justify-center shadow-lg">
                        <span className="text-blue-700 text-sm sm:text-base md:text-lg lg:text-xl font-bold">VS</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-1 justify-end min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
                      <div className="text-right min-w-0 flex-1 flex flex-col justify-center">
                        <div className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-bold text-gray-900 break-words leading-tight min-h-[2em] flex items-center justify-end">{game.awayTeam.name}</div>
                        <div className="text-[10px] sm:text-xs text-gray-500 font-medium uppercase tracking-wide mt-1">Extérieur</div>
                      </div>
                      {game.awayTeam.logo && (
                        <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 rounded-full bg-white p-1 sm:p-1.5 md:p-2 shadow-md border border-gray-100 flex-shrink-0 self-center">
                          <img
                ...
                {isLoadingBet ? (
                  ...
                  <div className="bg-white rounded-lg p-2 sm:p-3 md:p-4 border border-gray-200 shadow-md">
                    <div className="flex items-stretch justify-between space-x-2 sm:space-x-4 md:space-x-6">
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="flex gap-1 sm:gap-2 items-stretch flex-1">
                          <div className="flex-1 flex flex-col">
                            <label htmlFor="homeScore" className="hidden sm:block text-xs sm:text-sm font-medium text-gray-700 mb-1 text-center break-words leading-tight min-h-[2em] flex items-center justify-center">
                              {game.homeTeam.name}
                            </label>
                            <input
                              type="number"
                              ...
                              className="w-full rounded-lg border border-gray-300 shadow-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-200 text-center text-lg sm:text-xl md:text-2xl font-bold py-1.5 sm:py-2 md:py-3 transition-all duration-200 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                              placeholder="0"
                              required
                            />
                          </div>
                          <div className="relative flex-shrink-0 flex flex-col justify-end">
                            <select
                              ...
                              value=""
                              onChange={(e) => setHomeScore(e.target.value)}
                              className="w-12 sm:w-16 md:w-20 rounded-lg border border-gray-300 bg-white focus:border-primary-500 focus:ring-2 focus:ring-primary-200 cursor-pointer appearance-none text-xs sm:text-sm"
                            ...
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              <ChevronDownIcon className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-gray-500" />
                            </div>
                          </div>
                        </div>
                      </div>
                      
                      <div className="text-lg sm:text-xl md:text-2xl font-bold text-gray-400 px-2 sm:px-3 md:px-4 self-end pb-1.5 sm:pb-2 md:pb-3">-</div>
                      
                      {/* Away Score */}
                      <div className="flex-1 flex flex-col">
                        <div className="flex gap-1 sm:gap-2 items-stretch flex-1">
                          <div className="flex-1 flex flex-col">
                            <label htmlFor="awayScore" className="hidden sm:block text-xs sm:text-sm font-medium text-gray-700 mb-1 text-center break-words leading-tight min-h-[2em] flex items-center justify-center">
                              {game.awayTeam.name}
                            </label>
                            <input
                              type="number"
